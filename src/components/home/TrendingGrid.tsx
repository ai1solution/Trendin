...
import { TrendingUp, Zap, LineChart, BarChart3, X, ExternalLink, Sparkles } from 'lucide-react';
                                ...
                                onClick={() => {
                                    ...
                                    trackEvent({
                                        ...
                                        properties: {
                                            ...
                                            topic_difficulty: topic.difficulty || 'Med',
                                            niche: activeFilter
                                        }
                                    });
                                    generateDrafts(topic.title);
                                }}
                                className="group flex flex-col h-full bg-white border border-gray-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-xl transition-all duration-300 hover:-translate-y-1 cursor-pointer"
                            >
                                {/* Thumbnail Image */}
                                {topic.thumbnail_url && (
                                    <div className="relative w-full h-40 overflow-hidden bg-gray-100">
                                        ...
                                        <div className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 opacity-0 group-hover:opacity-100 transition-all duration-300">
                                            <div className="flex items-center gap-2 px-4 py-2 text-sm font-bold text-white bg-linkedin-600/90 backdrop-blur-sm rounded-full shadow-lg">
                                        ...
                                        {/* Hot Badge */}
                                        {idx < 3 && (
                                            <div className="absolute top-3 left-3 flex items-center gap-1 px-2 py-1 text-[10px] font-bold text-white bg-gradient-to-r from-energy-500 to-energy-600 rounded shadow-sm">
                                ...
                                <div className="flex flex-col flex-1 p-5">
                                    ...
                                    <div className="flex items-center justify-between mb-3">
                                        {topic.source_name ? (
                                            <div className="flex items-center gap-1.5 text-xs font-semibold text-linkedin-600">
                                                <div className="w-1.5 h-1.5 rounded-full bg-linkedin-500" />
                                                {topic.source_name}
                                            </div>
                                        ) : (
                                        ...
                                        {topic.published_date && (
                                            <span className="text-[10px] font-medium text-gray-400">
                                                {new Date(topic.published_date).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })}
                                            </span>
                                        )}
                                    </div>

                                    {/* Title */}
                                    <h3 className="mb-3 text-lg font-bold leading-snug text-gray-900 group-hover:text-linkedin-700 transition-colors line-clamp-2">
                                        {topic.title}
                                    </h3>
                                        ...
                                        <div className="flex items-center gap-3">
                                            ...
                                            {topic.link && (
                                                <a
                                                    ...
                                                    className="flex items-center gap-1 text-xs font-medium text-gray-400 hover:text-linkedin-600 transition-colors"
